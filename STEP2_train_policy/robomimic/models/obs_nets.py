...
from robomimic.models.base_nets import Module, Sequential, MLP, RNN_Base, ResNet18Conv, SpatialSoftmax, \
    FeatureAggregator, PointNet, SetTransformer, SetXFPCDEncoder, PointNet2
...
def obs_encoder_factory(
    ...
    for k, obs_shape in obs_shapes.items():
        ...
        if "pointcloud" in k:
            # ObsUtils.register_encoder_core(PointNet2)
            # enc_kwargs["core_class"] = 'PointNet2'
            # enc_kwargs["core_kwargs"] = {'input_dim': 6,
            #                             'output_dim': 64}
            
            # ObsUtils.register_encoder_core(SetTransformer)
            # enc_kwargs["core_class"] = 'SetTransformer'
            # enc_kwargs["core_kwargs"] = {'input_dim': 6,
            #                             'output_dim': 64,
            #                             'dim_hidden': 64,
            #                             'num_heads': 4}
