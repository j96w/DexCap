...
class BC_RNN(BC):
    ...
    def _create_networks(self):
        ...
        if self.obs_shapes["robot0_eef_pos"][0] == 3:
            self.arm_split = 3
            self.action_split = 6+1
        else:
            self.arm_split = 6
            self.action_split = 12+2
    ...
    def _compute_losses(self, predictions, batch):
        ...
        actions = predictions["actions"]
        losses["l2_loss"] = nn.MSELoss()(actions, a_target)
        losses["l1_loss"] = nn.SmoothL1Loss()(actions[..., :self.action_split], a_target[..., :self.action_split])
        # cosine direction loss on eef delta position
        losses["cos_loss"] = LossUtils.cosine_loss(actions[..., :self.arm_split], a_target[..., :self.arm_split])
